...
from predict_input import Predict_Input
    ...
    def get_input_fields(self, fields, xml):
        ...
        for item in activity.getElementsByTagName("EditText"):
            ...
            if android_id:
                ...
                name = fields[id]
                if input_type is not None and "NONE" not in input_type.upper():
                    input_fields[id] = input_type
                else:
                    print 'Input type not present. Using prediction for ', name
                    if name in self.predictions.keys():
                        input_fields[id] = self.predictions[name]
                        prediction_used = True
                    else:
                        print name, 'was not predicted'
                        input_fields[id] = '0x00000000'
            num = num + 1
        print 'Num elements found: ', num
        if prediction_used:
            relevant_file = open('relevant_files.db', 'a')
            relevant_file.write(self.apk + '\n')
            relevant_file.close()
        # print input_fields
        return input_fields
